    ...
    <div class="container">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <?php
                $result = mysqli_query($mysqli, "SELECT * FROM main ORDER BY status, tgl_awal");
                $no = 1;
...
if (isset($_POST['simpan'])) {
    if (isset($_POST['id'])) {
        $ubah = mysqli_query($mysqli, "UPDATE main SET
                                        isi = '" . $_POST['isi'] . "',
        ...
        $tambah = mysqli_query($mysqli, "INSERT INTO main(isi,tgl_awal,tgl_akhir,status)
                                        VALUES (
                                            '" . $_POST['isi'] . "',
                                            '" . $_POST['tgl_awal'] . "',
                                            '" . $_POST['tgl_akhir'] . "',
...
if (isset($_GET['aksi'])) {
    if ($_GET['aksi'] == 'hapus') {
        $hapus = mysqli_query($mysqli, "DELETE FROM main WHERE id = '" . $_GET['id'] . "'");
    } else if ($_GET['aksi'] == 'ubah_status') {
        $ubah_status = mysqli_query($mysqli, "UPDATE main SET status = '" . $_GET['status'] . "' WHERE id = '" . $_GET['id'] . "'");
    }
    echo "<script>
            document.location='index.php';
            </script>";
